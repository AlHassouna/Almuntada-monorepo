...
const Client: NextPage<Props> = (props) => {
    ...
    const userListURL = `${process.env.NEXT_PUBLIC_API_URL}/v1/events` || ''
    const [fallbackResource, setFallbackResource] = useState<Resource<IEvent>>(
        newResource([], 0, page, perPage),
    )
    const {isOpen, onClose, onOpen, onSubmit} = EventLogic();
    // swr: data -> axios: data -> resource: data
    const {data: {data: resource}} = useSWRAxios<Resource<IEvent>>({
        url: userListURL,
        params: {
            _page: page,
            _limit: perPage,
        ...
        transformResponse: transformResponseWrapper((d: IEvent[], h) => {
        ...
        <AdminLayout>
            ...
            <div style={{
                ...
                >
                    ...
                    {
                        isOpen &&
                        <EventForm onSubmit={onSubmit} handleClose={onClose} isOpen={isOpen}/>
                    }
                </div>
            </div>
        </AdminLayout>
